...
public class SourceWeiboView extends LinearLayout {
    ...
    private SimpleDraweeView weiboPic;
    ...
    private void init() {
        ...
        profile = (SimpleDraweeView) findViewById(R.id.iv_user_profile_in_source);
        ...
        weiboPic = (SimpleDraweeView) findViewById(R.id.retweet_weibo_picture_in_source);
    ...
    public void setView(WeiboInfo weibo) {
        ...
        createTime.setText(MumuWeiboUtility.parseWeiboTime(weiboInfo.getCreateTime()));
        if (!TextUtils.isEmpty(weiboInfo.getSourceName()))
            sourceFrom.setText("来自" + weiboInfo.getSourceName());


        user = weiboInfo.getWeiboUser();
        if (user != null) {
            profile.setImageURI(Uri.parse(user.getProfile()));
        ...
        if (pic == null || pic.equals(""))
            ...
            CommonUtils.draweeSupportGif(weiboPic, pic);
        }
